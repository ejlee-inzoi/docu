...
- **FootType** (`EB1FootType`): Foot classification (e.g., flat, heel).  
- **FootTypeMesh** (`TMap<EB1FootType, TSoftObjectPtr<USkeletalMesh>>`): Mesh mappings per foot type for compatibility.  
- **MatchingGenderId** (`TMap<EB1GenderType, FName>`): Gender-specific remap IDs. *(Foreign = "GarmentSocksMesh.Id")*  
- **MatchingBodyAgeId** (`TMap<EB1BodyAgeType, FName>`): Age group-specific remap IDs. *(Foreign = "GarmentSocksMesh.Id")*  
